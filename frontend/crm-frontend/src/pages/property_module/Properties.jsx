...
const Properties = () => {
  ...
  // ✅ 3. ROBUST FILTER LOGIC (Supports Admin & Manager)
  const getSafeId = (field) => {
    if (!field) return "";
    if (typeof field === "object" && field._id) return String(field._id).trim();
  ...
  const filteredProperties = properties.filter((p) => {
    ...
    // Use 'assignedTo' as the Agent field based on your Schema
    const assignedAgentId = getSafeId(p.assignedTo);
    const filterId = getSafeId(viewTargetId);

    if (role === "admin" || role === "manager") {
      if (filterId) {
        // Admin or Manager selected a specific person -> Show ONLY that person's work
        matchesRole = assignedAgentId === filterId;
      } else {
        // No filter -> Show Global (All work) or Team work
        matchesRole = true;
      }
    } else {
      // Agent Lock: Can only see properties assigned to THEM
      matchesRole = assignedAgentId === myId;
    }
