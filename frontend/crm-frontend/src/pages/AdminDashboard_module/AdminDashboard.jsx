...
import { 
  Users, ShieldCheck, TrendingUp, Trophy, ArrowUpRight, 
  Loader, RefreshCw, Save, CheckCircle2, AlertCircle, IndianRupee, PieChart
} from "lucide-react";
...
const AdminDashboard = () => {
  const [data, setData] = useState({
    stats: { totalRevenue: 0, totalProfit: 0, totalPipeline: 0, totalAgents: 0, totalManagers: 0 },
    managerPerformance: [],
    topAgents: [],
    managersList: [],
    agentsList: []
    ...
    try {
      ...
      res.data.managersList.forEach(mgr => {
        initialMapping[mgr._id] = res.data.agentsList
          .filter(agent => (agent.managedBy?._id || agent.managedBy) === mgr._id)
          .map(agent => agent._id);
      });
  ...
  const agentOwnershipMap = useMemo(() => {
    ...
    Object.entries(selectedAgents).forEach(([mgrId, agents]) => {
      agents.forEach(agentId => { map[agentId] = mgrId; });
    });
    return map;
  }, [selectedAgents]);

  const toggleSelection = (mgrId, agentId) => {
    const currentOwnerId = agentOwnershipMap[agentId];
    ...
    setSelectedAgents(prev => {
      ...
      const isRemoving = currentList.includes(agentId);
      return {
        ...prev,
        [mgrId]: isRemoving 
    ...
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.pageSubtitle}>Company-wide hierarchy and performance oversight</p>
        </div>
      ...
      <section className={styles.grid}>
        <div className={`${styles.card} ${styles.premiumCard}`}>
          <div className={styles.cardContent}>
            <div>
              ...
              <h3 className={styles.cardValue}>₹{(data.stats.totalRevenue/100000).toFixed(2)}L</h3>
            </div>
            <div className={styles.iconBox} style={{background: 'rgba(255,255,255,0.1)'}}>
              <TrendingUp size={24} />
            </div>
          </div>
        </div>
        ...
        <div className={styles.card}>
           ...
           <h3 className={styles.cardValue} style={{color: '#ea580c'}}>₹{(data.stats.totalPipeline/100000).toFixed(2)}L</h3>
           <span className={styles.cardNote}>In-progress Deals</span>
        </div>

        <div className={styles.card}>
           <p className={styles.cardTitle}>Total Profit</p>
           <h3 className={styles.cardValue} style={{color: '#16a34a'}}>₹{(data.stats.totalProfit/100000).toFixed(2)}L</h3>
           <span className={styles.cardNote}>20% Est. Margin</span>
        </div>

        <div className={styles.card}>
           <p className={styles.cardTitle}>Operations</p>
           <h3 className={styles.cardValue}>{data.stats.totalManagers}M / {data.stats.totalAgents}A</h3>
          ...
          <div style={{ width: '100%', height: 320 }}>
            <ResponsiveContainer>
              <BarChart data={data.managerPerformance}>
                ...
                <XAxis dataKey="managerName" axisLine={false} tickLine={false} />
                <YAxis axisLine={false} tickLine={false} tickFormatter={(val) => `₹${val/1000}k`} />
                <Tooltip 
                    cursor={{fill: '#f8fafc'}} 
                    formatter={(val) => `₹${val.toLocaleString()}`}
                />
                <Bar dataKey="teamProfit" fill="#4f46e5" radius={[6, 6, 0, 0]} barSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {data.topAgents.map((agent, index) => (
              <div key={agent._id} className={styles.leaderItem}>
                ...
                <div className={styles.agentMeta}>
                  ...
                  <p className={styles.rev}>₹{(agent.revenue || 0).toLocaleString()}</p>
                </div>
      ...
      <section className={styles.tableSection}>
        <div className={styles.tableHeader}>
           <h3><ShieldCheck size={20} color="#4f46e5"/> Team Assignment Matrix</h3>
            ...
            <tbody>
              {data.managersList.map(mgr => (
                <tr key={mgr._id}>
                  ...
                  <td>
                    <div className={styles.agentSelector}>
                      {data.agentsList.map(agent => {
                        const isActive = (selectedAgents[mgr._id] || []).includes(agent._id);
                        const assignedElsewhere = agentOwnershipMap[agent._id] && agentOwnershipMap[agent._id] !== mgr._id;
                        return (
                          <button 
                            key={agent._id}
                            className={`${styles.tag} ${isActive ? styles.activeTag : ""} ${assignedElsewhere ? styles.disabledTag : ""}`}
                            onClick={() => toggleSelection(mgr._id, agent._id)}
                            disabled={assignedElsewhere}
                          >
                            {agent.name} 
                            {isActive && <CheckCircle2 size={12} className={styles.checkIcon} />}
                            {assignedElsewhere && <span className={styles.busyLabel}>Busy</span>}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <td>
                    <button onClick={() => handleSaveTeam(mgr._id)} className={styles.saveBtn}>
